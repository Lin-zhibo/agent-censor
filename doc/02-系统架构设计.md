...
- 结果面板：3 秒内呈现审核结果，展示 `decision`、`labels`。
...
- 根据任务类型调用模型路由。
- 仅叶子智能体在最小标签判断前调用 Graph RAG，获取当前叶子标签邻域内的规则、样本和关键词知识证据。
- 根据历史上下文支持连续追问和策略解释。
...
- 向量库用于在当前叶子标签邻域内语义召回规则说明、样本和关键词知识节点。
- 图谱用于表达领域、分类、叶子标签、规则、样本、关键词之间的关系。
- 检索结果返回命中节点、关系路径、相似度或置信度、证据摘要。
- 审核解释应说明叶子智能体“为什么命中该标签”和“依据哪些规则、样本或知识节点”。
